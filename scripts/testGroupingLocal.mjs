...
import { findSimilarComplaints, groupSimilarComplaints } from '../backend/utils/complaintClustering.js';
...
const run = async () => {
    try {
        ...
        console.log('findSimilarComplaints result (2 km):', group ? { id: group.id, size: group.complaints.length } : null);

        const group5 = await findSimilarComplaints(c1, 5); // 5 km radius
        console.log('findSimilarComplaints result (5 km):', group5 ? { id: group5.id, size: group5.complaints.length } : null);

        // Also run the groupSimilarComplaints directly on array
        const groups = await groupSimilarComplaints([c1, c2], 2);
        console.log('groupSimilarComplaints direct result:', groups.map(g => ({ id: g.id, size: g.complaints.length })));
